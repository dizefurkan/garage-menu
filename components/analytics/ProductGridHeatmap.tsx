...
function getHeatmapColor(intensity: number): string {
  ...
  if (intensity < 0.25) return 'bg-green-100 border-green-300';
  if (intensity < 0.5) return 'bg-yellow-100 border-yellow-300';
  if (intensity < 0.75) return 'bg-orange-100 border-orange-300';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.map((product) => {
            ...
            return (
              ...
              >
                {/* Product Name */}
                <h4 className="font-semibold text-slate-800 truncate mb-2">
                  {product.productName}
                </h4>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <p className="font-semibold text-slate-800">{product.clicks}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">CTR</p>
                    <p className="font-semibold text-slate-800">{product.clickThroughRate.toFixed(1)}%</p>
                  </div>
                  <div>
                    <p className="text-slate-500">Viewers</p>
                    <p className="font-semibold text-slate-800">{product.uniqueViewers}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">Avg Time</p>
                    <p className="font-semibold text-slate-800">{product.avgTimeSeconds.toFixed(1)}s</p>
                  </div>
                </div>
